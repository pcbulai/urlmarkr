class UserController < ApplicationController
  ...
  def login
    ...
    if session[:user] = User.authenticate(params[:user][:login], params[:user][:password])
       ...
       logger.info(" user.remember_me #{@user.remember_me}")

       if @user.remember_me==1.to_s
            logger.info("Setting Cookie for user in login #{params[:user][:remember_me]}" )
            cookies[:LoginEngine]={ :value => params[:user][:login] , :expires => 30.days.from_now }
       end
       redirect_to :controller => 'bookmarks', :action => 'index'
    else
    ...
    begin
      User.transaction(@user) do
        ...
        if @user.save
          ...
          @preference = Preference.new(:created_by => @user.id)
          @preference.save
  ...
  def logout
    session[:user] = nil
    logger.info("Deleting Cookie for user in logout")
    cookies.delete :LoginEngine
    redirect_to :controller => 'bookmarks', :action => 'index'
  end
      ...
      begin
        User.transaction(user) do
          ...
          if user.save
              ...
              #flash["status"] = "Updated password emailed to #{@user.email}"
            #else
