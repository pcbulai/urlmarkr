  ...
  module AuthenticatedSystem
    ...
    def user?
      ...
      if id and key
       ...
       if cookies[:LoginEngine]
         logger.info(" cookies present, recover user from cookie ")     
          session[:user] = User.find(:first, :conditions => ["login = ? AND verified = 1 AND deleted = 0", cookies[:LoginEngine]])   
          return true if not session[:user].nil?
       end
      # Everything failed
      return false
